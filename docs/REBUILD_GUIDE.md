...
پلاک پذیرفته‌شده باید confidence کافی و فرمت معتبر ایرانی داشته باشد. Overlay حدود 2.5 ثانیه باقی می‌ماند. deduplication داخلی history دوربین برای متن یکسان در همان ROI و processing item کمتر از 5 ثانیه انجام می‌شود؛ UI ثبت دوبارهٔ همان متن را برای همان ROI و processing item همان دوربین تا 30 ثانیه suppress می‌کند. Runtime فقط ۱۵ رخداد تازهٔ هر دوربین/قابلیت را در حافظه نگه می‌دارد؛ موارد خارج‌شده در `history-archive.json` کنار executable با crop تصویر ذخیره می‌شوند و هنگام بازسازی history همراه رخدادهای حافظه تا سقف ۱۰۰ کارت نمایش داده می‌شوند.
