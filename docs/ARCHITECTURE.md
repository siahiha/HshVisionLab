...
Source → frame slot/queue → latest-preview slot → preview worker (≤15 FPS)
       ...
       → CameraPipelineCoordinator: registry → ساخت/اجرای pipelineهای ROI
...
- cooldown تاریخچهٔ تریگر در سطح `TriggerDefinition` است، نه دوربین. هنگام ارزیابی هر تریگر، `TriggerHistory` در SQLite با ایندکس `(TriggerId, TriggerKey, OccurredAtUtc)` بررسی می‌شود؛ کلید فقط اجزای الزامی سناریو را شامل می‌کند، بنابراین Plate-only به چهرهٔ اختیاری وابسته نیست و Plate+Face به جفت هر دو هویت وابسته است. ردیف تاریخچه هم‌زمان با رخداد canonical و در همان transaction ثبت و هنگام حذف رخداد پاک می‌شود.
