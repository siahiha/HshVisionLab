...
- `camera-wall` با 1، 2، 3 یا 4 ستون؛ تعداد ستون از `Math.min(4,
  Math.max(1, list.length))` می‌آید؛
...
`SnapshotImage` از endpoint زیر استفاده می‌کند و پس از هر load با تأخیر 85ms
درخواست بعدی می‌سازد؛ خطا با تأخیر 500ms دوباره تلاش می‌شود:
