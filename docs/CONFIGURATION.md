...
| Plate | `Options: PlateProcessingOptions` به‌همراه `MaxFps` و `Threads` | شامل `ModelFile`، `InputSize`، `Preprocessing`، `Confidence`، `NmsIoU` و `TrackMaxMisses` است. در UI مدل به‌صورت ComboBox از catalog سرویس انتخاب می‌شود، نه text input. `ModelFile` نام منطقی ONNX است؛ runtime ابتدا مدل package شدهٔ هم‌نام را در `Models/Plate` و سپس در `Models` کنار executable جست‌وجو می‌کند و مسیر قدیمی `Modules/Plate/Models` را نیز برای سازگاری می‌پذیرد. در Debug، `HshDetectionEngin.Plate/Models` نیز fallback است. |
...
| Face model / input | `face_yunet_2023mar.onnx` / `320` |
