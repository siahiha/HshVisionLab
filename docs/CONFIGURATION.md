...
| حذف رخداد تکراری | `DuplicateEventCooldownSeconds` | در سطح هر دوربین، رخداد canonical تکراری را بر اساس دوربین، ROI، پلاک و هویت چهره تا این تعداد ثانیه suppress می‌کند؛ مقدار `0` غیرفعال است و مقدار پیش‌فرض `60` ثانیه است. این suppression قبل از Event Store، SignalR و trigger اعمال می‌شود. |
